...
public class ProductRatePlanQueryHandler implements IQueryHandler {

	@Override
	public Object getById(String id) throws CommandException, NotFoundException {
		ProductRatePlan ratePlan = (ProductRatePlan) HibernateUtils.get(ProductRatePlan.class, id);
		if (ratePlan == null) {
			throw new NotFoundException("Product rate plan id(" + id + ") not found.");
		}
		if (ratePlan.getStatus() == Flags.EntityStatus.DELETED.toString())
			throw new NotFoundException("Product rate plan id(" + id + ") not found.");
	...
	public List<Object> getProductRatePlan(Query q) {
		...
		List<Object> ratePlans = HibernateUtils.select("FROM ProductRatePlan" + whereSQL, sqlParams);
		return ratePlans;
	}
