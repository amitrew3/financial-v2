...
public class TransactionDeductionQueryHandler implements IQueryHandler {
    ...
    public List<Object> get(Query q) {
        ...
        if (q.has("transactionId")) {
            builder.append("AND");
            builder.append(DB.Field.TransactionDeduction.TRANSACTION_ID);
            builder.append("= :transactionId ");
            sqlParams.put("transactionId", q.get("transactionId"));
        }
        if (q.has("deductionId")) {
            builder.append("AND");
            builder.append(DB.Field.TransactionDeduction.DEDUCTION_ID);
            builder.append("= :deductionId ");
            sqlParams.put("deductionId",q.get("deductionId"));
        }

        int offset = 0;
        offset = (limit * (page - 1));

        List<Object> associates = HibernateUtilV2.select("FROM TransactionAssociate " + builder.getValue(), sqlParams, q.getQuery(), limit, offset);
        return associates;
    }
